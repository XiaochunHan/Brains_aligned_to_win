...
p<-ggplot(dataset_pseudo, aes(x = (perm_acc-acc_all), fill = as.factor(ci95))) +
    ...
    geom_vline(aes(xintercept = 0), color = "#4C4C4C", linewidth = 1) + 
    labs(x = "Decreased Accuracy") + theme(text = element_text(size = 25),legend.position="none",axis.ticks.length=unit(-0.1, "cm"))
print(p)
...
p<-ggplot(dataset_circular, aes(x = (perm_acc-acc_all), fill = as.factor(ci95))) +
    ...
    geom_vline(aes(xintercept = 0), color = "#4C4C4C", linewidth = 1) + theme_classic()+xlim(-0.2,0.1) + 
    labs(x = "Decreased Accuracy") + theme(text = element_text(size = 25),legend.position="none",axis.ticks.length=unit(-0.1, "cm"))
print(p)
...
### Import data
```{r import_data}
setwd('/Users/xiaochunhan/Documents/Research/Projects/DaFD/P02_Data/D01_all_feature_each_trial/Study1-DaFC')
wd<-getwd()
dataset_BOLD <- read.csv('BOLD_single_trial.csv',header=TRUE,sep=",")
dataset_FC <- read.csv('FC_single_trial.csv',header=TRUE,sep=",")
...
p_match_inter_perm = mean(meanACC_inter<acc_match_inter_perm)
...
p_match_intra_perm = mean(meanACC_intra<acc_match_intra_perm)
print(p_match_intra_perm)

p <- ggplot(acc_match_intra_perm, aes(x = acc_perm, fill = as.factor(ci95))) +
    geom_histogram(colour = "#4C4C4C", bins = 61) + 
    scale_fill_manual(values = c("white","#B095C6"))+
    geom_vline(aes(xintercept = meanACC_intra), color = "#B095C6", linewidth = 2) + theme_classic()+xlim(0,1) + 
    labs(x = "Win-Lose Prediction Accuracy") + theme(text = element_text(size = 15),legend.position="none",axis.ticks.length=unit(-0.1, "cm"))
print(p)
